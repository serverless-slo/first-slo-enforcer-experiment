...
def generate_requests(pool, idle_time, acc, srate, erate, hduration, lduration, period, stepsize):
    global sentRequests
    ...
    while period > 0:
        ...
        # Stablize for a while
        n = int(rate * hduration)
        for i in range(n):
            res = pool.apply_async(func=reqfunc)
            pendingRequests.put(res)
            sentRequests.inc()
...
if __name__ == '__main__':
    ...
    repeat = 2
    if len(sys.argv) != 9:
        eprint("Usage: python genburst.py [parallelism] [srate] [erate] [acceleration] [high rate duration] [low rate duration] [repeat] [uniform|memoryless|gaussian]")
        exit(1)
    else:
        ...
        hduration = int(sys.argv[5])
        lduration = int(sys.argv[6])
    ...
    sender = threading.Thread(target=generate_requests, args=(pool, idle_time, acceleration, srate, erate, hduration, lduration, repeat, 1))
    sender.start()
